...
class CliffordAlgebra(nn.Module):
    def __init__(self, metric):
        ...
        device = 'cuda'  # Assuming you are using the first GPU
        self.metric = torch.as_tensor(metric).to(device)

        # self.learned_metric = nn.Parameter(torch.full_like(self.metric_base, fill_value=1e-6))
        # self.e = nn.Parameter(torch.tensor(1.0))
        # self.metric = self.metric_base + self.learned_metric
