    ...
    def __init__(
        ...
        if layer_type == "fc":
            self.phi_x = nn.Sequential(
                ...
                MVLayerNorm(self.algebra, self.metric, hidden_features_x),
            )
            ...
            self.theta_x = nn.Sequential(
                ...
                MVLayerNorm(self.algebra, self.metric, out_features_x),
            )
...
class LorentzCGGNN(nn.Module):
    ...
    def __init__(
        ...
        # time.sleep(5)    
        # self.algebra = CliffordAlgebra((1.0, -1.0, -1.0, -1.0)).to('cuda')
        # expanded_metric = 0.5 * torch.diag(self.algebra.metric)
        # print("*"*60)
        # print("Using Fixed Metric")
        # self.metric = nn.Parameter(expanded_metric, requires_grad=False)
        # print("*"*60)
        # time.sleep(5)    
        ...
        print("*"*60)
        self.metric = nn.Parameter(expanded_metric + 1e-7*torch.rand(4, 4).to('cuda'), requires_grad=False)
        time.sleep(5)    
    ...
    def forward(self, data, batch_idx):
        ...
        P = P.real
        atom_momentum = torch.matmul(atom_momentum, P)
