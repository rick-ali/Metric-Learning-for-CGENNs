...
from algebra.norms import cayley


class SteerableGeometricProductLayer(nn.Module):
    def __init__(
        self, algebra, metric, features, include_first_order=True, normalization_init=0
    ):
        super().__init__()
        ...
        if normalization_init is not None:
            self.normalization = NormalizationLayer(
                algebra, self.metric, features, normalization_init
            )
        else:
    ...
    def _get_weight(self):
        ...
        return cayley(self.algebra, torch.linalg.eig(self.metric)[0].real) * weight_repeated
