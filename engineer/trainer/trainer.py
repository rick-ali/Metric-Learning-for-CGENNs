...
class Trainer:
    def __init__(
        ...
        self.counter = 0  # Needed for early stopping
        self.patience = 3  # Needed for early stopping
        self.early_stop_reached = False
    ...
    def save_metric(self, model):
        full_file_path = os.path.join(self.SAVE_PATH_2)
        print(f"Saving metric at {full_file_path}")

        if not hasattr(model, 'metric'):
            raise ValueError("The model does not have a 'metric' attribute to save.")

        metric_string = str(model.metric.data.cpu().numpy() + model.metric.data.cpu().numpy().T)
        os.makedirs(os.path.dirname(full_file_path), exist_ok=True)
        
        # Determine the file opening mode based on whether it's the first save operation
        mode = 'w' if self.first_metric_save else 'a'
        with open(full_file_path, mode) as file:
            file.write(metric_string + '\n')
        
        # After the first save operation, set the flag to False so subsequent saves will append
        self.first_metric_save = False
    ...
    def test_loop(
        ...
        if validation:
            ...
            if self.early_stop(metrics["val/loss"]):
                self.early_stop_reached = True


    @torch.no_grad()
    def early_stop(self, validation_loss):
        if validation_loss < self.min_validation_loss:
            print(f"Current validation loss {validation_loss}, best validation loss = {self.min_validation_loss}")
            self.min_validation_loss = validation_loss
            self.counter = 0
        elif validation_loss > (self.min_validation_loss):
            print(f"Validation loss did not get better! {self.patience - self.counter} attempts left")
            print(f"Current validation loss {validation_loss}, best validation loss = {self.min_validation_loss}")
            self.counter += 1
    ...
    def get_val_loss(
        ...
        metrics = self._add_prefix(metrics, "val")
    ...
    def should_stop(self):
        ...
        if self.max_steps is not None and self.global_step >= self.max_steps:
            ...
            return True
        # if self.early_stop_reached:
        #     print("Stopping due to early_stopping.")
        #     return True
        return False
    ...
    def fit(
        ...
        # Incorporate the random number into your save paths
        self.SAVE_PATH = os.path.join("models", "saved_models", f"{MODEL_NAME}_{random_number}")
        self.SAVE_PATH_2 = os.path.join("models", "saved_metrics", MODEL_NAME, f"metric_log_{random_number}.txt")

        enable_grad_step = int(self.max_steps * 0.01) 
        ...
        while not self.should_stop:
            ...
            for batch in train_loader:
                ...
                # Enable learning for the metric
                if self.global_step == enable_grad_step:
                    print("Enabling learning for metric parameter")
                    model.metric.requires_grad = True 
